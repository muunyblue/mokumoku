...
この記事は、[Three.js Advent Calendar 2016](http://qiita.com/advent-calendar/2016/threejs "Three.js Advent Calendar 2016 - Qiita") 1日目の記事です。[まんがタイムきらら Advent Calendar 2016](http://www.adventar.org/calendars/1454#list-2016-12-05 "まんがタイムきらら Advent Calendar 2016 - Adventar")ではありません。

---

仕事で疲れた状態でステラのまほうエンディングのヨナカジカルを見ているとWebGLでやれそうだなぁとなったので仕事の息抜きにやってみた話です。
...
以下、最後に書いてあるhtmlとjavascript部分をちょっといじってファイルにしたものです。
...
実際に動かしてみると正方形がぐるぐる回っていますが、表示したいのはただの四角形なので四角形をドキュメントから探します。
...
BufferかBufferじゃないか判断するためBufferGeometryのドキュメントを読みます。
...
BufferGeometryはGeometryよりも効率的だけど扱いが大変で、Shaderとか使う場合はBufferGeomerty一択になるようです。
...
あと、cameraのz方向がサンプルコードのままだと近すぎて何も見えないので設定したfar程度に離します。

(fovやfrustumなど全く聞き慣れない単語なので要検索です。[画角 - Wikipedia](https://ja.wikipedia.org/wiki/%E7%94%BB%E8%A7%92 "画角 - Wikipedia")や[6. カメラ（processing 3D入門） | Yasushi Noguchi Class](http://r-dimension.xsrv.jp/classes_j/6_interactive3d/ "6. カメラ（processing 3D入門） | Yasushi Noguchi Class")を参考にしました。)
